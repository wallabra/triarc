"""Snobe is a tiny PRAW (Reddit) wrapper library, that provides
facilities such as learning from and replying to Reddit posts
using the COBE chatbot library."""
...
try:
    from . import snobeconfig as CFG
...
class Snobe:
    """A class responsible for accessing Reddit."""

    def __init__(self, brain_file: str, **kwargs):
        """
        Arguments:
            brain_file {str} -- The filename of the file to use as the COBE brain file.
        """
        ...
        cur = self.sql()
        cur.execute(
            'CREATE TABLE IF NOT EXISTS LearnedComments (subm_id STRING, comm_id STRING);'
        )
        cur.execute(
            'CREATE TABLE IF NOT EXISTS CheckedComments '
            '(subm_id STRING, comm_id STRING, reply STRING);'
        )
        self.sql_save()

    def step(self):
        if not self.reply_queue.empty() and (
                not self.last_reply or time.time() - self.last_reply >= self.reply_interval
        ):
    ...
    def _reply(self, model, reply):
        if isinstance(model, praw.models.Comment):
            cur = self.sql()
            cur.execute(
                'INSERT INTO CheckedComments VALUES (?, ?, ?);',
                (model.submission.id, model.id, reply)
            )
            self.sql_save()

        res = model.reply(
            re.sub(r'(?<=\W)/?u/[^\W]+(?=\W)', '<anonymous person>', reply) + (
                '\n\n    *Beep boop. I\'m a bot!* If something goes wrong, please tell {}...'
                .format(CFG.AUTHOR)
            )
        )
    ...
    def check_learn(self, *submissions):
        ...
        for submission in submissions:
            ...
            for i, comment in enumerate(submission.comments):
                cur.execute(
                    'SELECT * FROM LearnedComments WHERE subm_id = ? AND comm_id = ?;',
                ...
                if not cur.fetchone():
                    ...
                    cur.execute(
                        'INSERT INTO LearnedComments VALUES (?, ?);',
                        (submission.id, comment.id)
                    )
    ...
    def check_reply(self, *submissions):
        ...
        for submission in submissions:
            ...
            for i, comment in enumerate(submission.comments):
                cur.execute(
                    'SELECT * FROM CheckedComments WHERE subm_id = ? AND comm_id = ?;',
                    (submission.id, comment.id)
                )
        ...
        for submission in submissions:
            ...
            for i, comment in enumerate(submission.comments):
                ...
                if not cur.fetchone():
                    ...
                    print(
                        "- Composed simulated reply from {} ({}/{})".format(comment.id, i + 1, them)
                    )
                    print(rep)
                    print()
...
def main():
    if len(sys.argv) <= 2:
        print(
            'Provide either "learn", "reply", "learn-many" or "reply-all" as a command,'
            'and a list of Reddit submission URLs to either just learn from, or learn and reply.'
        )
        return False

    snobe = Snobe(
        os.environ.get('BRAIN', 'brains/reddit.brain'),
    ...
    if command == 'learn':
        ...
        if len(args) < 2 or not args[1].isdigit():
            print(
                "\"learn-many\" requires a subeddit name and a number of posts to read, "
                "and no individual links."
            )
            return False
