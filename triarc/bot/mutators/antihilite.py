...
class AntiHilite(Mutator):
    ...
    async def on_join(self, bot: Bot, which: Backend, event: IRCResponse):
        ...
        if "!" in event.origin and event.params.args[0] in "#+!":
            # Register this nick.
            self.nick_sets.setdefault((which, event.params.args[0]), set()).add(
                event.origin.split("!")[0]
            )
    ...
    async def on_part(self, bot: Bot, which: Backend, event: IRCResponse):
        ...
        if "!" in event.origin and event.params.args[0] in "#+!":
            ...
            nick_set = self.nick_sets.setdefault((which, event.params.args[0]), set())
    ...
    def registered(self, bot: Bot):
        ...
        async def check_names(
            bot: Bot, backend: Backend, kind: str, event: IRCResponse
        ):
            """
            ...
            if kind == "_NUMERIC" and event.is_numeric and event.kind == "353":
                ...
                for nick in nicks:
                    ...
                    self.nick_sets.setdefault((backend, channel), set()).add(nick)
    ...
    def modify_message(self, backend: Backend, target: str, message: str) -> str:
        ...
        for nick in nicks:
            if (
                nick in re.split(r"[^a-zA-Z0-9\-\[\]\_]+", message)
                and nick != backend.nickname
            ):
                return None
